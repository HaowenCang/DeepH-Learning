...
**M4：非正交基与周期表示**、**M5：DFT 与标签语义**、**M6：普通 MPNN 与周期图**和 **M7：群表示与等变网络**已经完成。M7-11 阶段 E 正式独立总审计为 `PASS`、问题为 0；D-011/M7-I 的全量审计经修复和同一审计员定点复核后也为 `PASS`。M8 已冻结，M9 已由 D-014 单独取得执行授权；M9-01—03 已通过。M9-04 的官方 ZIP、安全解包、逐文件清单和 450 结构合同已经完成，但非正交发布包缺 overlap，触发 `M9-DATA-B01`。用户通过 D-017 授权受限 overlap-only OpenMX 路线。工作包第四次定点复核将 B07 收敛到 `overlap-init` 的冻结解释器身份检查；主 agent 已完成第五轮公共 bootstrap 修复，等待同一审计员复核。复核通过前未安装、解压、编译、生成输入或计算，也未启动训练。Hamiltonian、SCF、其他 DFT 标签和自动重启仍未授权。
