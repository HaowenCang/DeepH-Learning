"""V5-specific chain regression. All writes are temporary; no compilation."""
import copy
...
t = load('v5_chain_fixtures', HERE / 'test_m9_source_build_v5_consumer.py')
c, i = t.c, t.i
LAUNCHER_PATH = c.LAUNCHER
l = load('v5_chain_launcher', LAUNCHER_PATH)
...
class ChainTests(unittest.TestCase):
    ...
    def actual_installed_fixture(self, chain=False, failure=False):
        ...
        values = {k: getattr(c, k) for k in ['PREPARED_SHA','RETIRED_SHA','EVIDENCE_SHA',
                                            'RECOVERY_EVIDENCE_SHA','FACT_SHA','PARENT_FORMAL_SHA',
                                            'PARENT_RUNTIME_SHA','PRESERVED_V4_AUTHORITY',
                                            'OLD_INSTALL_VERDICT_SHA','OLD_V3_INSTALL_VERDICT_SHA',
                                            'OLD_V4_INSTALL_VERDICT_SHA',
                                            'LOCK_EXPECTED']}
...
'''
        ...
        f.frozen = {'schema':'m9-source-build-frozen-v5','files':{str(p):c.sha(p.read_bytes()) for p in c.SOURCE_PATHS}}
        t.jwrite(c.FROZEN,f.frozen)
        f.verdict['frozen_sha256']=c.sha(c.FROZEN.read_bytes());t.jwrite(c.VERDICT,f.verdict)
        installed_verdict=dict(f.verdict,schema='m9-source-build-installation-verdict-v5',
        ...
        manifest={'schema':'m9-source-build-snapshot-v5','members':{name:{'bytes':len(data),'sha256':c.sha(data)} for name,data in payloads.items()}}
...
"""
    ...
    def test_live_bound_common_derived_driver_receipt_and_parent_chain(self):
        ...
        self.assertEqual(receipt['v5_binding'],c.execution_binding())
    ...
    def test_actual_loader_keeps_common_context_and_derives_only_build(self):
        repair = load('v5_test_repair', REPAIR_PATH)
    ...
    def test_real_uid1000_v5_loader_complete_toolchain_readonly(self):
...
'''
    ...
    def test_budget_uses_new_launcher_and_receipt_validator(self):
        b = c.load_bytes('v5_actual_budget', c.OLD_BUDGET, c.derive_budget(t.BASE))
        c.guard_budget(b)
    ...
    def test_missing_recovery_marker_rejected(self):
        ...
        with mock.patch.object(c, 'runtime_and_history', return_value=f.runtime):
            ...
            value = json.loads(path.read_bytes()); value.pop('recovered_from_source_build_wsl_shutdown')
            t.jwrite(path, value)
            with self.assertRaisesRegex(SystemExit, 'terminal state'): c.verify_gate(False)
    ...
    def receipt_fixture(self):
        ...
        receipt = dict(schema_version='m9-overlap-source-launcher-receipt-v1', status='PASS',
            capability_id=capability_id, transaction_id=transaction_id, budget_bootstrap=bootstrap, v5_binding=binding,
    ...
    def test_parent_rejects_old_missing_or_false_derived_provenance(self):
        ...
        x=copy.deepcopy(r);x.pop('v5_binding');variants.append(x)
        x=copy.deepcopy(r);x['v5_binding']['operation_id']='a237ec1f2b404d898840c643e9ae501b';variants.append(x)
        for x in variants:
    ...
    def test_real_budget_postchild_authority_drift_commits_failure_and_cpu(self):
        f=self.fixture()
        b=c.load_bytes('v5_budget_postchild',c.OLD_BUDGET,c.derive_budget(t.BASE))
