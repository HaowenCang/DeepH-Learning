...
CONSUMER_PATH = REPRO / 'controllers/m9_source_build_v5_consumer.py'
INSTALL_PATH = REPRO / 'controllers/m9_source_build_v5_install.py'
...
REAL_LAUNCHER = c.read_bytes(c.LAUNCHER)
REAL_REPAIR = c.read_bytes(c.REPAIR, c.REPAIR_SHA)
REAL_COMMON_PATH = c.PROJECT / '06_reproduction/scripts/m9_overlap_common.py'
REAL_DRIVER_PATH = c.DRIVER
...
class Fixture:
    def __init__(self, installed=True):
        ...
        self.overrides = {'PROJECT': project, 'ROOT': root, 'SNAPSHOT': root / 'controls/source-build-v5',
        ...
        names = ('PROJECT_CONSUMER', 'PROJECT_INSTALLER', 'FROZEN', 'AUTHORIZATION', 'WORK',
                 ...
                 'RECOVERY_EVIDENCE', 'FACT', 'RECEIPTS', 'OLD_CONSUMER_MANIFEST', 'OLD_OVERLAP_MANIFEST',
                 'OLD_BUILD_MANIFEST', 'OLD_INSTALL_VERDICT', 'CONTRACT', 'LAUNCHER', 'REPAIR',
                 'DECISION', 'AUTH_REVIEW', 'OLD_V2_MANIFEST', 'REPAIR_MANIFEST',
                 'OLD_V3_MANIFEST', 'HOST_RECOVERY_MANIFEST', 'OLD_V3_INSTALL_VERDICT',
                 'OLD_V4_MANIFEST', 'WSL_RECOVERY_MANIFEST', 'OLD_V4_INSTALL_VERDICT', 'CHAIN_TEST')
        ...
        sources = {getattr(c, name) for name in ('PROJECT_CONSUMER', 'PROJECT_INSTALLER', 'TEST',
            'WORK', 'AUTHORIZATION', 'EVIDENCE', 'RECOVERY_EVIDENCE', 'FACT', 'RECEIPTS', 'OLD_CONSUMER_MANIFEST', 'OLD_OVERLAP_MANIFEST',
        ...
        write(c.REPAIR, REAL_REPAIR)
        write(c.LAUNCHER, REAL_LAUNCHER)
        for p in (c.OLD_CONSUMER_MANIFEST, c.OLD_OVERLAP_MANIFEST, c.OLD_BUILD_MANIFEST,
                  c.OLD_V2_MANIFEST, c.REPAIR_MANIFEST, c.OLD_V3_MANIFEST, c.HOST_RECOVERY_MANIFEST,
                  c.OLD_V4_MANIFEST, c.WSL_RECOVERY_MANIFEST):
            jwrite(p, {'files': {}})
        old_manifests = {p: c.sha(p.read_bytes()) for p in (c.OLD_CONSUMER_MANIFEST,
            c.OLD_OVERLAP_MANIFEST,c.OLD_BUILD_MANIFEST,c.OLD_V2_MANIFEST,c.REPAIR_MANIFEST,
            c.OLD_V3_MANIFEST,c.HOST_RECOVERY_MANIFEST,c.OLD_V4_MANIFEST,c.WSL_RECOVERY_MANIFEST)}
        ...
        self.stack.enter_context(mock.patch.object(c, 'OLD_V3_INSTALL_VERDICT_SHA', c.sha(c.OLD_V3_INSTALL_VERDICT.read_bytes())))
        self.stack.enter_context(mock.patch.object(c, 'OLD_V4_INSTALL_VERDICT_SHA', c.sha(c.OLD_V4_INSTALL_VERDICT.read_bytes())))
        ...
        state = {'hard_stopped': False, 'active_overlap_transaction': None,
                 'recovered_from_source_build_wsl_shutdown': c.RECOVERY_ID,
        ...
        tx = {'state': 'FAILED_COMMITTED', 'action': 'source_build',
              'transaction_id': 'd52575f446d3c62f0fc93c3c65f3c959',
              'recovery_id':c.RECOVERY_ID,'reasons':['wsl_shutdown_external_interruption'],
              'elapsed_seconds':138.401802566,
        ...
        for p in [root / 'controls', root / 'manifests']:
            initial.update(self.reader.capture_tree(p))
        for index in range(301-len(initial)):
            write(root/'controls'/('parent-padding-%03d' % index),b'parent\n')
        for p in [root / 'controls', root / 'manifests']:
            formal.update(self.reader.capture_tree(p))
        raw=json.dumps(formal,ensure_ascii=False,sort_keys=True,separators=(',',':')).encode('utf-8')
        compressed=gzip.compress(raw)
        parent_runtime={name:c.sha((root/'manifests'/name).read_bytes()) for name in
            ('budget_state.json','overlap_workflow_state.json','overlap_transaction.json','budget_ledger.jsonl')}
        self.stack.enter_context(mock.patch.object(c, 'PARENT_RUNTIME_SHA', parent_runtime))
        self.stack.enter_context(mock.patch.object(c, 'PARENT_FORMAL_SHA', c.sha(raw)))
        preserved={'consumed_capability_sha256':'a'*64,'gate_sha256':'b'*64,
                   'permit_sha256':'c'*64,'launcher_receipt_present':False}
        self.stack.enter_context(mock.patch.object(c, 'PRESERVED_V4_AUTHORITY', preserved))
        self.stack.enter_context(mock.patch.object(c, 'RECOVERY_EVIDENCE_SHA', c.sha(c.RECOVERY_EVIDENCE.read_bytes())))
        self.stack.enter_context(mock.patch.object(c, 'FACT_SHA', c.sha(c.FACT.read_bytes())))
        self.baseline = {'schema':'m9-source-build-v5-parent-namespace-evidence-v1',
            'captured_utc':'2026-09-03T00:00:00Z',
            'capture_mode':'root-fixed-python3.9.23-I-S-B-shared-budget-lock-double-capture',
            'receipt_source_sha256':c.RECEIPTS_SHA,
            'recovery_execution_fact_report':{'path':str(c.FACT),'sha256':c.FACT_SHA},
            'recovery_postexecution_evidence':{'path':str(c.RECOVERY_EVIDENCE),'sha256':c.RECOVERY_EVIDENCE_SHA},
            'formal_namespace': {'count':301,'second_capture_equal':True,
                'canonical_json_bytes':len(raw),'canonical_json_gzip_bytes':len(compressed),
                'canonical_sha256':c.sha(raw),
                'canonical_json_gzip_base64':base64.b64encode(compressed).decode('ascii')},
            'prepared_tree':{'path':str(c.PREPARED),'count':5319,
                'canonical_sha256':c.PREPARED_SHA,'second_capture_equal':True},
            'runtime_sha256':{str(root/'manifests'/name):digest for name,digest in parent_runtime.items()},
            'preserved_v4_authority':preserved,
            'lock_receipt':dict(zip(('dev','ino','uid','gid','mode','nlink','bytes'),c.LOCK_EXPECTED)),
        ...
        self.frozen = {'schema': 'm9-source-build-frozen-v5', 'files': {
        ...
        self.verdict = {'schema': 'm9-source-build-implementation-verdict-v5', 'status': 'PASS',
        ...
        snapshot = {'schema': 'm9-source-build-snapshot-v5', 'members': {
        ...
        self.gate = {'schema': 'm9-source-build-readiness-v5', 'status': 'PASS', 'scope': c.SCOPE,
        ...
        iv = dict(self.verdict, schema='m9-source-build-installation-verdict-v5',
                  report_path=str(c.INSTALL_REPORT), report_sha256=c.sha(c.INSTALL_REPORT.read_bytes()))
        jwrite(c.INSTALL_VERDICT, iv)
        self.permit = {'schema': 'm9-source-build-execution-permit-v5',
...
class BuildTests(unittest.TestCase):
    ...
    def test_mechanical_permit_requires_fact_and_rejects_reissue(self):
        ...
        verdict = dict(f.verdict, schema='m9-source-build-installation-verdict-v5',
                       report_path=str(c.INSTALL_REPORT), report_sha256=c.sha(c.INSTALL_REPORT.read_bytes()))
    ...
    def test_parent_namespace_envelope_requires_exact_301_object_identity(self):
        f = self.fixture()
        self.assertEqual(len(c.baseline_namespace(f.baseline)), 301)
        variants = []
        x = json.loads(json.dumps(f.baseline)); x['formal_namespace']['count'] = 300; variants.append(x)
        x = json.loads(json.dumps(f.baseline)); x['formal_namespace']['canonical_sha256'] = '0' * 64; variants.append(x)
        x = json.loads(json.dumps(f.baseline)); x['formal_namespace']['canonical_json_gzip_base64'] = 'not-base64'; variants.append(x)
        namespace = c.baseline_namespace(f.baseline)
        namespace.pop(next(iter(namespace)))
        raw = json.dumps(namespace, ensure_ascii=False, sort_keys=True, separators=(',', ':')).encode('utf-8')
        compressed=gzip.compress(raw)
        x=json.loads(json.dumps(f.baseline));x['formal_namespace']={'count':301,'second_capture_equal':True,
            'canonical_json_bytes':len(raw),'canonical_json_gzip_bytes':len(compressed),
            'canonical_sha256':c.sha(raw),
            'canonical_json_gzip_base64':base64.b64encode(compressed).decode('ascii')};variants.append(x)
        for value in variants:
            with self.subTest(value=value['formal_namespace']):
                with self.assertRaises(SystemExit): c.baseline_namespace(value)

    def test_new_parent_execution_evidence_and_audit_drift_are_rejected_zero_runtime_write(self):
        for target in ('EVIDENCE', 'RECOVERY_EVIDENCE', 'FACT'):
            with self.subTest(target=target):
                f = Fixture()
                try:
                    path = getattr(c, target); write(path, path.read_bytes() + b'drift\n')
                    before = f.reader.capture_tree(c.ROOT)
    ...
    def run_uid1000(self, f, command='verify-function'):
        ...
        settings={'paths':path_values,'SOURCE_PATHS':[str(p) for p in c.SOURCE_PATHS],
            ...
            'PREPARED_SHA':c.PREPARED_SHA,'RETIRED_SHA':c.RETIRED_SHA,'EVIDENCE_SHA':c.EVIDENCE_SHA,
            'RECOVERY_EVIDENCE_SHA':c.RECOVERY_EVIDENCE_SHA,'FACT_SHA':c.FACT_SHA,
            'PARENT_FORMAL_SHA':c.PARENT_FORMAL_SHA,'PARENT_RUNTIME_SHA':c.PARENT_RUNTIME_SHA,
            'PRESERVED_V4_AUTHORITY':c.PRESERVED_V4_AUTHORITY,
            'OLD_INSTALL_VERDICT_SHA':c.OLD_INSTALL_VERDICT_SHA,
            'OLD_V3_INSTALL_VERDICT_SHA':c.OLD_V3_INSTALL_VERDICT_SHA,
            'OLD_V4_INSTALL_VERDICT_SHA':c.OLD_V4_INSTALL_VERDICT_SHA,
            'LOCK_EXPECTED':c.LOCK_EXPECTED}
...
for k in ['PREPARED_SHA','RETIRED_SHA','EVIDENCE_SHA','RECOVERY_EVIDENCE_SHA','FACT_SHA','PARENT_FORMAL_SHA','PARENT_RUNTIME_SHA','PRESERVED_V4_AUTHORITY','OLD_INSTALL_VERDICT_SHA','OLD_V3_INSTALL_VERDICT_SHA','OLD_V4_INSTALL_VERDICT_SHA']:setattr(c,k,x[k])
...
"""
    ...
    def test_v3_gate_permit_operation_and_nonce_are_not_v5_authority(self):
        old_operation = '7bc4139f7a9c4b768d3f9ec68c1d1048'
        old_nonce = 'a930f2b8e7ac449bb251482012eb99d1c7032df764985aaac34519bf6db2013a'
        f=Fixture()
        try:
            old=dict(f.gate,schema='m9-source-build-readiness-v3',
                operation_id=old_operation, nonce=old_nonce)
        ...
        f=Fixture()
        try:
            old=dict(f.permit,schema='m9-source-build-execution-permit-v3',
                operation_id=old_operation, nonce=old_nonce)
            jwrite(c.PERMIT,old,0o640)
            result=self.run_uid1000(f,'verify-execution')
            self.assertNotEqual(result.returncode,0)
            self.assertIn('build execution permit binding differs',result.stderr)
        finally:
            f.close()
