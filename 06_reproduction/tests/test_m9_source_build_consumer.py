...
CONSUMER_PATH = REPRO / 'controllers/m9_source_build_consumer.py'
INSTALL_PATH = REPRO / 'controllers/m9_source_build_install.py'
...
class Fixture:
    def __init__(self, installed=True):
        ...
        self.overrides = {'PROJECT': project, 'ROOT': root, 'SNAPSHOT': root / 'controls/source-build-v1',
        ...
        names = ('PROJECT_CONSUMER', 'PROJECT_INSTALLER', 'FROZEN', 'AUTHORIZATION', 'WORK',
                 ...
                 'FACT', 'RECEIPTS', 'OLD_CONSUMER_MANIFEST', 'OLD_OVERLAP_MANIFEST', 'CONTRACT')
        ...
        for p in (c.OLD_CONSUMER_MANIFEST, c.OLD_OVERLAP_MANIFEST):
            jwrite(p, {'files': {}})
        old_manifests = {p: c.sha(p.read_bytes()) for p in (c.OLD_CONSUMER_MANIFEST, c.OLD_OVERLAP_MANIFEST)}
        ...
        self.frozen = {'schema': 'm9-source-build-frozen-v1', 'files': {
        ...
        self.verdict = {'schema': 'm9-source-build-implementation-verdict-v1', 'status': 'PASS',
        ...
        snapshot = {'schema': 'm9-source-build-snapshot-v1', 'members': {
        ...
        self.gate = {'schema': 'm9-source-build-readiness-v1', 'status': 'PASS', 'scope': c.SCOPE,
            ...
            'authorization_sha256': c.sha(c.AUTHORIZATION.read_bytes()), 'runtime': self.runtime}
        jwrite(c.GATE, self.gate, 0o640)
        write(c.INSTALL_REPORT, b'independent installation PASS\n')
        iv = dict(self.verdict, schema='m9-source-build-installation-verdict-v1',
                  report_path=str(c.INSTALL_REPORT), report_sha256=c.sha(c.INSTALL_REPORT.read_bytes()))
        jwrite(c.INSTALL_VERDICT, iv)
        self.permit = {'schema': 'm9-source-build-execution-permit-v1',
...
class BuildTests(unittest.TestCase):
    ...
    def test_mechanical_permit_requires_fact_and_rejects_reissue(self):
        ...
        verdict = dict(f.verdict, schema='m9-source-build-installation-verdict-v1',
                       report_path=str(c.INSTALL_REPORT), report_sha256=c.sha(c.INSTALL_REPORT.read_bytes()))
...
for k in ['PREPARED_SHA','RETIRED_SHA','EVIDENCE_SHA','FACT_SHA']:setattr(c,k,x[k])
...
"""
        result=subprocess.run(['/usr/sbin/runuser','-u','evan-williams','--',sys.executable,'-I','-S','-B',
            '-c',code,str(CONSUMER_PATH),json.dumps(settings)],capture_output=True,text=True)
