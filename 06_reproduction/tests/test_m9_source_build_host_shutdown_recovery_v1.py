"""Isolated tests for the host-shutdown recovery. No formal recovery is called."""
import base64
import gzip
...
SOURCE=PROJECT/'06_reproduction/controllers/m9_source_build_host_shutdown_recovery_v1.py'
spec=importlib.util.spec_from_file_location('shutdown_recovery',SOURCE)
...
reader=r.load_source('shutdown_recovery_receipts_fixture',helper,r.pinned(helper,r.RECEIPTS_SHA))


def runtime_raw():
    state={'active_overlap_transaction':r.FAILURE_ID,'hard_stopped':False,
        'cpu_seconds':{'overlap_build':7214.469142588001,'overlap_smoke':0.0,'overlap_batch':0.0},
    ...
    tx={'schema_version':'m9-overlap-transaction-v1','transaction_id':r.FAILURE_ID,'state':'RUNNING',
        'action':'source_build','bucket':'overlap_build','child_pid':396,
        'start_utc':'2026-08-31T15:19:49.210200Z','keep':'transaction'}
...
class Fixture:
    def __enter__(self):
        self.temp=tempfile.TemporaryDirectory(prefix='m9-shutdown-recovery-test-')
        root=Path(self.temp.name); self.root=root
        overrides={'ROOT':root/'runtime','PRIVATE':root/'private/recovery'}
        ...
        for path in (r.ROOT/'manifests/overlap_capabilities',r.ROOT/'controls',r.ROOT/'software',
                     r.ROOT/'logs',r.PRIVATE.parent): path.mkdir(parents=True,exist_ok=True)
        ...
        for index in range(221-len(initial)):
            (r.ROOT/'controls'/('padding-%03d' % index)).write_bytes(b'fixture\n')
        self.expected=r.formal_namespace(reader)
        if len(self.expected)!=221: raise AssertionError('fixture formal namespace must contain 221 objects')
        formal_raw=json.dumps(self.expected,ensure_ascii=False,sort_keys=True,
                              separators=(',',':')).encode('utf-8')
        products={str(r.ROOT/'env/forbidden-product'):False}
        self.evidence={'status':'INTERRUPTED_NONTERMINAL','blocking':1,'non_blocking':0,
            'formal_count':len(self.expected),'conservative_elapsed_upper_bound_seconds':r.ELAPSED,
            'formal_namespace_encoding':'canonical-json-gzip-base64',
            'formal_namespace_json_bytes':len(formal_raw),'formal_namespace_sha256':r.sha(formal_raw),
            'formal_namespace_json_gzip_base64':base64.b64encode(gzip.compress(formal_raw)).decode('ascii'),
            'runtime_raw_bytes':{str(path):{'base64':base64.b64encode(data).decode('ascii'),
                'bytes':len(data),'sha256':r.sha(data)} for path,data in self.raw.items()},
            'consumed_capability':self.capability,
            'permit_receipt':{'sha256':r.sha(r.PERMIT.read_bytes())},
            'logs':{str(path):reader.filesystem_receipt(path)
                    for path in (r.LOGS,*r.LOGS.iterdir())},'products_present':products}
...
class RecoveryTests(unittest.TestCase):
    def test_real_frozen_authority_closure_with_strict_fixture_verdict(self):
        with tempfile.TemporaryDirectory(prefix='m9-shutdown-authority-') as directory:
            ...
            value={'schema':'m9-host-shutdown-recovery-implementation-verdict-v1','status':'PASS',
    ...
    def test_targets_preserve_history_and_charge_conservative_upper_bound(self):
        with Fixture() as f:
            ...
            self.assertAlmostEqual(state['cpu_seconds']['overlap_build'],7507.245045788001,places=10)
            ...
            self.assertEqual(tx['reasons'],['host_shutdown_external_interruption'])
    ...
    def test_nonformal_drift_between_validation_and_first_write_is_zero_write_rejected(self):
        variants=('namespace','build','clean','log','product','process','storage')
        for variant in variants:
            with self.subTest(variant=variant),Fixture() as f,fixture_authority(f):
                ...
                if variant=='namespace': (r.ROOT/'manifests/unknown-object').write_bytes(b'unknown')
                    ...
                    product=Path(next(iter(f.evidence['products_present'])))
    ...
    def test_zero_write_fails_and_preserves_target(self):
        with Fixture() as f:
            ...
            self.assertTrue(r.STATE.with_name(r.STATE.name+'.host-shutdown-recovery.tmp').exists())
    ...
    def test_process_detection_uses_exact_original_argv(self):
        capability={'budget_argv':['python','formal-parent'],'launcher_argv':['python','formal-child']}
        with mock.patch.object(Path,'iterdir',return_value=[]): self.assertFalse(r.process_alive(capability))
