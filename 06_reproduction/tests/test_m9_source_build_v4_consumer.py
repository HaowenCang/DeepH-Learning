...
CONSUMER_PATH = REPRO / 'controllers/m9_source_build_v4_consumer.py'
INSTALL_PATH = REPRO / 'controllers/m9_source_build_v4_install.py'
...
class Fixture:
    def __init__(self, installed=True):
        ...
        self.overrides = {'PROJECT': project, 'ROOT': root, 'SNAPSHOT': root / 'controls/source-build-v4',
        ...
        names = ('PROJECT_CONSUMER', 'PROJECT_INSTALLER', 'FROZEN', 'AUTHORIZATION', 'WORK',
                 ...
                 'OLD_V3_MANIFEST', 'HOST_RECOVERY_MANIFEST', 'OLD_V3_INSTALL_VERDICT', 'CHAIN_TEST')
        ...
        sources = {getattr(c, name) for name in ('PROJECT_CONSUMER', 'PROJECT_INSTALLER', 'TEST',
            ...
            'HOST_RECOVERY_MANIFEST', 'OLD_V3_INSTALL_VERDICT', 'CHAIN_TEST')}
        ...
        old_manifests = {p: c.sha(p.read_bytes()) for p in (c.OLD_CONSUMER_MANIFEST,
            ...
            c.OLD_V3_MANIFEST,c.HOST_RECOVERY_MANIFEST)}
        ...
        state = {'hard_stopped': False, 'active_overlap_transaction': None,
                 'recovered_from_source_build_host_shutdown': c.RECOVERY_ID,
        ...
        tx = {'state': 'FAILED_COMMITTED', 'action': 'source_build',
              'transaction_id': '4d7808af7df9418518a59afeba766eb9',
              'recovery_id':c.RECOVERY_ID,'reasons':['host_shutdown_external_interruption']}
        ...
        for index in range(244-len(initial)):
        ...
        self.baseline = {'formal_namespace': {'count':244,'second_capture_equal':True,
            'canonical_json_bytes':len(raw),'canonical_sha256':c.sha(raw),
            'canonical_json_gzip_base64':base64.b64encode(gzip.compress(raw)).decode('ascii')}}
        ...
        self.frozen = {'schema': 'm9-source-build-frozen-v4', 'files': {
        ...
        self.verdict = {'schema': 'm9-source-build-implementation-verdict-v4', 'status': 'PASS',
        ...
        snapshot = {'schema': 'm9-source-build-snapshot-v4', 'members': {
        ...
        self.gate = {'schema': 'm9-source-build-readiness-v4', 'status': 'PASS', 'scope': c.SCOPE,
        ...
        iv = dict(self.verdict, schema='m9-source-build-installation-verdict-v4',
                  report_path=str(c.INSTALL_REPORT), report_sha256=c.sha(c.INSTALL_REPORT.read_bytes()))
        jwrite(c.INSTALL_VERDICT, iv)
        self.permit = {'schema': 'm9-source-build-execution-permit-v4',
...
class BuildTests(unittest.TestCase):
    ...
    def test_mechanical_permit_requires_fact_and_rejects_reissue(self):
        ...
        verdict = dict(f.verdict, schema='m9-source-build-installation-verdict-v4',
                       report_path=str(c.INSTALL_REPORT), report_sha256=c.sha(c.INSTALL_REPORT.read_bytes()))
    ...
    def test_parent_namespace_envelope_requires_exact_244_object_identity(self):
        f = self.fixture()
        self.assertEqual(len(c.baseline_namespace(f.baseline)), 244)
        variants = []
        x = json.loads(json.dumps(f.baseline)); x['formal_namespace']['count'] = 243; variants.append(x)
        ...
        variants.append({'formal_namespace': {'count': 244, 'second_capture_equal': True,
            'canonical_json_bytes': len(raw), 'canonical_sha256': c.sha(raw),
            'canonical_json_gzip_base64': base64.b64encode(gzip.compress(raw)).decode('ascii')}})
...
for k in ['PREPARED_SHA','RETIRED_SHA','EVIDENCE_SHA','FACT_SHA','OLD_INSTALL_VERDICT_SHA','OLD_V3_INSTALL_VERDICT_SHA']:setattr(c,k,x[k])
...
"""
    ...
    def test_v3_gate_permit_operation_and_nonce_are_not_v4_authority(self):
