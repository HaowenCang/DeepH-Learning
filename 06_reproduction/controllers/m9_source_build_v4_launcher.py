...
SNAPSHOT = Path('/home/evan-williams/deeph-m9/controls/source-build-v4')
CONSUMER_NAME = 'm9_source_build_v4_consumer.py'
SELF_NAME = 'm9_source_build_v4_launcher.py'
CONSUMER = None
REPAIR = None
V4_BINDING = None
...
def pinned(path, expected=None):
    ...
    try:
        ...
        if (not stat.S_ISREG(before.st_mode) or before.st_nlink != 1
            ...
            raise RuntimeError('v4 installed source identity differs: ' + str(path))
        return data
    finally:
        os.close(fd)
...
def installed_authority():
    ...
    if set(manifest) != {'schema', 'members'} or manifest['schema'] != 'm9-source-build-snapshot-v4':
        raise RuntimeError('v4 launcher snapshot schema differs')
    ...
    spec = importlib.util.spec_from_file_location('m9_v4_launch_consumer', SNAPSHOT / CONSUMER_NAME)
    ...
    if (binding['consumer_sha256'] != sha256_bytes(data)
        ...
        raise RuntimeError('v4 launcher self-consumer binding differs')
    repair_path = SNAPSHOT / c.REPAIR.name
    repair = c.load_bytes('m9_v4_launch_repair', repair_path, pinned(repair_path, c.REPAIR_SHA))
    CONSUMER, REPAIR, V4_BINDING = c, repair, binding
    return binding
...
class FrozenSourceLoader(importlib.abc.Loader):
    ...
    def exec_module(self, module: object) -> None:
        if CONSUMER is None or REPAIR is None:
            raise ImportError('v4 source load requires installed authority')
        ...
        if self.fullname == 'm9_openmx_build':
            if self.path != CONSUMER.DRIVER or base_sha != CONSUMER.DRIVER_BASE_SHA:
                raise ImportError('v4 driver base identity differs')
...
def wait_and_consume_capability(capability_id: str) -> dict[str, object]:
    ...
    if CONSUMER is None or CONSUMER.execution_binding() != V4_BINDING:
        raise RuntimeError('v4 execution binding changed before capability consumption')
    ...
    if (parent_argv != parent_expected or value.get('action') != 'source_build'
        ...
        raise RuntimeError('v4 capability action or budget parent differs')
    consumed = dict(value)
...
def main() -> int:
    ...
    if len(sys.argv) != 3 or sys.argv[1] != '--capability-id' or not re.fullmatch(r'[0-9a-f]{32}', sys.argv[2]):
        raise RuntimeError('v4 launcher requires exact capability argv')
