...
SNAPSHOT = ROOT / 'controls/source-build-v4'
...
PROJECT_CONSUMER = PROJECT / '06_reproduction/controllers/m9_source_build_v4_consumer.py'
PROJECT_INSTALLER = PROJECT / '06_reproduction/controllers/m9_source_build_v4_install.py'
FROZEN = PROJECT / '06_reproduction/manifests/m9_source_build_v4_frozen_hashes.json'
AUTHORIZATION = PROJECT / '08_audits/M9_standing_execution_authorization_20260831.md'
WORK = PROJECT / '08_audits/M9_source_build_v4_work_package.md'
TEST = PROJECT / '06_reproduction/tests/test_m9_source_build_v4_consumer.py'
CHAIN_TEST = PROJECT / '06_reproduction/tests/test_m9_source_build_v4_chain.py'
REPORT = PROJECT / '08_audits/M9_source_build_v4_implementation_audit.md'
VERDICT = PROJECT / '08_audits/M9_source_build_v4_implementation_verdict.json'
INSTALL_REPORT = PROJECT / '08_audits/M9_source_build_v4_installation_audit.md'
INSTALL_VERDICT = PROJECT / '08_audits/M9_source_build_v4_installation_verdict.json'
EVIDENCE = PROJECT / '08_audits/M9_source_build_host_shutdown_recovery_v1_postexecution_evidence.json'
FACT = PROJECT / '08_audits/M9_source_build_host_shutdown_recovery_v1_execution_independent_audit.md'
...
OPERATION = 'bb0b7c8b982d4d72a66a1f01f95533da'
...
LAUNCHER = PROJECT / '06_reproduction/controllers/m9_source_build_v4_launcher.py'
...
EVIDENCE_SHA = '42e05a042ab66c41b31e03ee9a126eec58a1480c382834bd07cd9244f88d602a'
FACT_SHA = '41fc9940fa8f7ac66b7d8978e429e7b41c3f3f1753e2b2c37c59148f36829c78'
PREPARED_SHA = '411a8a674559723f0616deafbe67c93d3e31453461fa6b2af26d4135bb3e1705'
...
def baseline_namespace(baseline):
    ...
    if (not isinstance(formal, dict) or formal.get('count') != 244
            or not formal.get('second_capture_equal')
            or not isinstance(formal.get('canonical_json_gzip_base64'), str)):
        raise SystemExit('v4 parent namespace envelope differs')
    try:
        raw = gzip.decompress(base64.b64decode(
            formal['canonical_json_gzip_base64'], validate=True))
        value = json.loads(raw)
    except Exception as exc:
        raise SystemExit('v4 parent namespace payload is invalid') from exc
    if (len(raw) != formal.get('canonical_json_bytes')
            or sha(raw) != formal.get('canonical_sha256')
            or canonical(value) != formal.get('canonical_sha256')
            or len(value) != 244):
        raise SystemExit('v4 parent namespace identity differs')
    return value
...
def history_attestation(baseline):
    namespace = baseline_namespace(baseline)
    return canonical({'schema':'m9-source-build-history-attestation-v4',
...
def verify_sources(frozen):
    if set(frozen) != {'schema', 'files'} or frozen['schema'] != 'm9-source-build-frozen-v4':
...
def read_snapshot(gate):
    ...
    if set(manifest) != {'schema', 'members'} or manifest['schema'] != 'm9-source-build-snapshot-v4':
    ...
    verify_verdict(json.loads(payloads[VERDICT.name]), 'm9-source-build-implementation-verdict-v4',
...
def validate_gate(gate):
    if (set(gate) != {'schema', 'status', 'scope', 'action', 'operation_id', 'nonce',
            ...
            or gate['schema'] != 'm9-source-build-readiness-v4'
...
def verify_gate(require_permit=True):
    ...
    if (state['hard_stopped'] or workflow['hard_stopped']
            ...
            or state.get('recovered_from_source_build_host_shutdown') != RECOVERY_ID
            or workflow.get('recovered_from_source_build_host_shutdown') != RECOVERY_ID
            or tx['state'] != 'FAILED_COMMITTED' or tx['action'] != 'source_build'
            or tx['transaction_id'] != '4d7808af7df9418518a59afeba766eb9'
            or tx.get('recovery_id') != RECOVERY_ID
            or tx.get('reasons') != ['host_shutdown_external_interruption']):
    ...
    if require_permit:
        ...
        if (set(permit) != {'schema', 'gate_sha256', 'operation_id', 'nonce',
                ...
                or permit['schema'] != 'm9-source-build-execution-permit-v4'
        ...
        verify_verdict(verdict, 'm9-source-build-installation-verdict-v4',
...
def validate_launcher_receipt(capability_id, transaction_id, budget_bootstrap):
    ...
    try:
        ...
        raise ValueError('v4 post-child authority validation failed: ' + str(exc)) from exc
...
def _validate_launcher_receipt(capability_id, transaction_id, budget_bootstrap):
    ...
    if (receipt.get('schema_version') != 'm9-overlap-source-launcher-receipt-v1'
            ...
            or receipt.get('v4_binding') != binding
        ...
        raise ValueError('v4 launcher receipt/capability/authority mismatch')
    ...
    if (not isinstance(bootstrap, dict) or bootstrap.get('isolated') is not True
        ...
        raise ValueError('v4 launcher bootstrap provenance mismatch')
    ...
    for name, value in sources.items():
        ...
        if name == 'm9_openmx_build':
            ...
            if value.get('base_sha256') != DRIVER_BASE_SHA or frozen[path] != DRIVER_BASE_SHA:
                raise ValueError('v4 driver derivation provenance mismatch')
        if (not isinstance(value, dict) or set(value) != fields or value.get('path') != path
            ...
            raise ValueError('v4 actual loaded source mismatch: ' + name)
