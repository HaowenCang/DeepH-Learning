...
SELF = REPRO / 'controllers/m9_source_build_v2_install.py'
CONSUMER = REPRO / 'controllers/m9_source_build_v2_consumer.py'
FROZEN = REPRO / 'manifests/m9_source_build_v2_frozen_hashes.json'
REPORT = AUDITS / 'M9_source_build_v2_implementation_audit.md'
VERDICT = AUDITS / 'M9_source_build_v2_implementation_verdict.json'
...
def load_authority(args):
    ...
    if (frozen.get('schema') != 'm9-source-build-frozen-v2'
            or frozen.get('files', {}).get(str(SELF)) != self_sha
            or frozen.get('files', {}).get(str(CONSUMER)) != consumer_sha
            or verdict != {'schema': 'm9-source-build-implementation-verdict-v2',
    ...
    c.verify_verdict(verdict, 'm9-source-build-implementation-verdict-v2', frozen_sha, REPORT, report_sha)
...
def install(c, frozen, frozen_bytes, verdict_bytes, report_bytes, do_write):
    ...
    manifest = {'schema': 'm9-source-build-snapshot-v2', 'members': {
        name: {'bytes': len(data), 'sha256': c.sha(data)} for name, data in sorted(payloads.items())}}
    manifest_bytes = json_bytes(manifest)
    gate = {'schema': 'm9-source-build-readiness-v2', 'status': 'PASS', 'scope': c.SCOPE,
        ...
        'authorization_sha256': frozen['files'][str(c.AUTHORIZATION)],
        'history_attestation_sha256': c.history_attestation(baseline), 'runtime': runtime}
...
def permit(c, frozen, fact_verdict_sha, fact_report_sha):
    ...
    c.verify_verdict(verdict, 'm9-source-build-installation-verdict-v2', frozen_sha,
    ...
    value = {'schema': 'm9-source-build-execution-permit-v2',
