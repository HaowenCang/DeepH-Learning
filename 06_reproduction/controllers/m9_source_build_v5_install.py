...
SELF = REPRO / 'controllers/m9_source_build_v5_install.py'
CONSUMER = REPRO / 'controllers/m9_source_build_v5_consumer.py'
FROZEN = REPRO / 'manifests/m9_source_build_v5_frozen_hashes.json'
REPORT = AUDITS / 'M9_source_build_v5_implementation_audit.md'
VERDICT = AUDITS / 'M9_source_build_v5_implementation_verdict.json'
...
def load_authority(args):
    ...
    if (frozen.get('schema') != 'm9-source-build-frozen-v5'
            or frozen.get('files', {}).get(str(SELF)) != self_sha
            or frozen.get('files', {}).get(str(CONSUMER)) != consumer_sha
            or verdict != {'schema': 'm9-source-build-implementation-verdict-v5',
    ...
    c.verify_verdict(verdict, 'm9-source-build-implementation-verdict-v5', frozen_sha, REPORT, report_sha)
...
def install(c, frozen, frozen_bytes, verdict_bytes, report_bytes, do_write):
    ...
    manifest = {'schema': 'm9-source-build-snapshot-v5', 'members': {
        name: {'bytes': len(data), 'sha256': c.sha(data)} for name, data in sorted(payloads.items())}}
    manifest_bytes = json_bytes(manifest)
    gate = {'schema': 'm9-source-build-readiness-v5', 'status': 'PASS', 'scope': c.SCOPE,
...
def permit(c, frozen, fact_verdict_sha, fact_report_sha):
    ...
    c.verify_verdict(verdict, 'm9-source-build-installation-verdict-v5', frozen_sha,
    ...
    value = {'schema': 'm9-source-build-execution-permit-v5',
